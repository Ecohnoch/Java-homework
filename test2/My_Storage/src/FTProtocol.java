...
public class FTProtocol implements IOStrategy {
	...
	String VolumeOf(int index)
	{
		if(index<0)
			return ""+0;
		else return storage_servers[index].volume;
	}
	...
	int ChooseStorageServer(int exclusion,long file_len)
	{
		...
		for(int i=0;i<storage_num;i++)
		{
			...
			if(i!=exclusion&&storage_servers[i].usable==true&&SpaceEnough((storage_servers[i]).volume,file_len)>=0&&VolumeCompare(VolumeOf(i),VolumeOf(pre)))
			{
				pre=i;
			}
		}
	...
	boolean rename(int index_server,String target_file_name,String target_file_uuid,String new_name) throws Exception
	{
		//建立连接，并获得流
		...
		try {
			...
			case 2:	// file download  文件下载
				...
				else
				{
					...
					//实现选择存储服务器
					//两个结点都不可用的情况
					if(storage_servers[index_server1].usable==false&&storage_servers[index_server2].usable==false)
						dos.writeInt(-1);
					else if(storage_servers[index_server1].usable==true)		//优先用第一个
					{
						...
						try{
							r=rr=0;
							Socket s = new Socket(storage_servers[index_server1].ip,storage_servers[index_server1].port);
						...
						catch(Exception e)
						{
							...
							{
								Socket s = new Socket(storage_servers[index_server2].ip,storage_servers[index_server2].port);
			...
			case 3:			//remove    删除节点上的文件
				...
				else			//找到了
				{
					...
					//两个存储结点都在线，直接删除
					if(index_server1!=-1&&storage_servers[index_server1].usable==true&&index_server2!=-1&&storage_servers[index_server2].usable==true)	
					{
					...
					else if(index_server2==-1&&index_server1!=-1&&storage_servers[index_server1].usable==true)
					{
						remove(index_server1,target_file_name,target_file_uuid);
					}
					//文件在之前并没有完成备份存储的,只有一个节点保有数据，对其进行删除
					else if(index_server1==-1&&index_server2!=-1&&storage_servers[index_server2].usable==true)
					{
						remove(index_server2,target_file_name,target_file_uuid);
					}
