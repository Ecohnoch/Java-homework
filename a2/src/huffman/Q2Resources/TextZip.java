...
/**
 * 可以为这个类添加额外的方法及数据成员. ID就是指学号, 下面的作者一定要写上你的名字和学号 作业中出现的示范数据abdc001需要改成学生的学号数据
 * 
...
public class TextZip {
	 ...
	 *            counted pw the PrintWriter which is used to produce the output
	 *            text file listing the character frequencies
	 *
	...
	public static ArrayList countFrequencies(FileReader fr, PrintWriter pw) throws Exception {
		...
		for (int i = 0; i < f.length; i++) {
			if (f[i] != 0) {
				CharFreq cf = new CharFreq((char) i, f[i]);
				ans.add(new TreeNode(cf));
				pw.println((char) i + " " + f[i]);
			}
		}
		return ans;
	 ...
	 *
	 * @param inputFreqFile
	 *            the frequency file which stores characters and their frequency
	...
	public static ArrayList readFrequencies(String inputFreqFile) throws Exception {
		...
		while ((c2 = (char) br.read()) != -1) {
			...
			ans.add(new TreeNode(new CharFreq(c2, i)));
		}
		br.close();
		br = null;
		return ans;
	 ...
	 * 
	 * QUESTION 2 PART 1 ================= -a : this uses a default prefix code
	 ...
	 * 
	 * QUESTION 2 PART 2 ================= -f : given a text file (args[1]) and
	 ...
	 * 
	 * QUESTION 2 PART 3 ================= -c : given a text file (args[1]) and
	 * the name of an output frequency file (args[2]) and the name of the output
	 * compressed file (args[3]), this should compress file
	 * 
	 * QUESTION 2 PART 4 ================= -d : given a compressed file
