...
public class FTClient {
	...
	public void start(String server, int port) throws Exception {
		...
		if (args[1].equals("get")) {	//下载
			...
			if (c.equalsIgnoreCase("q")) {	//退出
				s.close();
				System.exit(0);
			}

			if (c.equalsIgnoreCase("a")) {	//下载全部
			...
			s.close();
			System.exit(0);
		} else if (args[1].equals("put")) {		//文件上传
	...
	public void upload(String filename) throws Exception {
		...
		//Encrypt file = null;
		// file.encrypt(filename, filename);		//文件加密
	...
	public void download(String filename) throws Exception {
		//Encrypt file = null;
		//file.decrypt(filename, filename);		//文件解密
		dos.writeInt(2);
